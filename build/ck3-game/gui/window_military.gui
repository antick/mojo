				...
				widget_header_with_picture = {
					...
					vbox = {
						...
						hbox = {
							...
							button_standard = {
								size = { 70 32 }
								datacontext = "[MilitaryView.GetDefaultRaidIntent]"
								visible = "[Character.CanRaid]"

								onclick = "[ToggleGameViewData( 'raid_intent_selection_window', GetPlayer )]"

								tooltip = "RAID_OPEN_INTENT_WINDOW"
...
types ArmiesView
{
	...
	type hbox_your_levies_and_knights = hbox
	{
		button_standard_clean = {
			name = "levies_button"
			datacontext = "[MilitaryView.GetComposition]"
			visible = "[Not( Or( IsLandlessAdventurer( GetPlayer ), IsNomad( GetPlayer ) ) )]"
		...
		vbox = {
			name = "nomad_riders_button"
			datacontext = "[MilitaryView.GetComposition]"
			datacontext = "[GetMaA( 'nomadic_riders' )]"
			...
			tooltipwidget = maa_type_container_tooltip
			using = tooltip_ws

			background = {
				texture = "gfx/interface/illustrations/men_at_arms_big/nomadic_riders.dds"
				alpha = 0.7
				fittype = centercrop
			...
			hbox = {
				...
				text_single = {
					layoutpolicy_horizontal = expanding
					text = "[GetPlayer.GetDomicile.GetHorde|0]"
					using = Font_Size_Big
					default_format = "#high"
					align = nobaseline
				...
				vbox = {
					...
					hbox = {
						...
						button_standard = {
							...
							button_plus = {
								name = "can_be_created"
								visible = "[MilitaryItem.MaACanBeCreated]"
								parentanchor = vcenter
								alwaystransparent = yes
								position = { 5 0 }
							}
						...
						button_standard = {
							...
							onclick = "[Character.OpenPlayerInteractionWithTargetTitle('request_title_troops_back_interaction', Title.Self)]"
...
types MilitaryView
{
	type army_automation_on_flat = widget
	{
		size = { 36 36 }
	...
	type army_automation_on = icon {
		...
		modify_texture = {
			name = "spin"
			texture = "gfx/interface/icons/army_status/army_status_automation.dds"
			blend_mode = Normal
			rotate_uv = 0
		}
	...
	type army_automation_on_big = icon {
		size = { 30 30 }
		texture = "gfx/interface/colors/black.dds"
		visible = "[Army.IsControlledByCompanionAI]"
		...
		modify_texture = {
			name = "spin"
			texture = "gfx/interface/icons/army_status/army_status_automation_big.dds"
			blend_mode = Normal
			rotate_uv = 0
		}
	...
	type button_toggle_companion_ai = button_standard {
		size = { 40 40 }

		onclick = "[PostCommand( GetToggleCompanionAICommand )]"
		down = "[GetPlayer.HasCompanionAI]"

		tooltip = "MV_AUTOMATION_TOGGLE_TT"
		using = tooltip_sw

		button_army_automation = {
			parentanchor = center
			alwaystransparent = yes
			size = { 32 32 }
	...
	type container_maa_item = container {
		...
		tooltipwidget = {
			...
			blockoverride "extra_info" {
				...
				text_multi = {
					layoutpolicy_horizontal = expanding
					visible = "[MenAtArmsType.GetSubjectProvider( GetPlayer ).IsValid]"
